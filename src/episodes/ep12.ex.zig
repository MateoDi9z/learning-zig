...
// You can dfine as many as you need
const NumberError = error{ InvalidCharacter, Overflow };

// Merge them together with the `||` operator
...
pub fn main() ParseError!void {
    ...
    result = try parseNumber(input); // Shortcut with try
    std.debug.print("result: {!}\n", .{result});
    std.debug.print("\n", .{});
    ...
    // `while` loops work with error unions similar to optionals.
    count_down = 3;
