...
pub fn main() !void {
    ...
    // Condition is comptime-known, so it's evaluated at compile time
    if (condition) {
        @compileError("What ?");
    }
...
// A tagged union with fields of the adove struct types
const U = union(enum) {
    a: A,
    b: B,

    // Using inline else to handle all variations in one shot
